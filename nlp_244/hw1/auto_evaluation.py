...
if __name__ == "__main__":
    ...
    y_true, y_true_intents = (
        opener("hw1_tags_test.txt")
        if args.test
        else opener("hw1_labels_dev.txt")
    )
    y_pred, y_pred_intents = (
        opener("prediction.txt") if args.test else opener("prediction_dev.txt")
    )

    print(
        "Sentence Accuracy is {:.2f}%".format(
            accuracy_score(y_true, y_pred) * 100
        )
    )
    print("Slot F1 Score is {:.2f}%".format(f1_score(y_true, y_pred) * 100))
    print(
        "Intent F1 Score is {:.2f}%".format(
